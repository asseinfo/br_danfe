...
describe BrDanfe::DanfeNfeLib::Helper do
  describe '.format_date' do
    it 'returns a formated string' do
      string = '2013-10-18T13:54:04'
      expect(BrDanfe::DanfeNfeLib::Helper.format_date(string)).to eq '18/10/2013'
    end

    describe 'when the source is blank' do
      it 'is empty' do
        expect(BrDanfe::DanfeNfeLib::Helper.format_date('')).to eq ''
      end
    end
  end
  ...
  describe '.format_time' do
    describe 'when param is a complete datetime' do
      ...
      it 'is a formated time string in localtime' do
        expect(BrDanfe::DanfeNfeLib::Helper.format_time(param)).to eq '16:54:04'
      end
    end
    ...
    describe 'when param is only a time' do
      ...
      it 'is a formated time string' do
        expect(BrDanfe::DanfeNfeLib::Helper.format_time(param)).to eq '14:23:02'
      end
    end
    ...
    describe 'when param is blank' do
      ...
      it 'is empty' do
        expect(BrDanfe::DanfeNfeLib::Helper.format_time(param)).to eq ''
      end
    end
  end
  ...
  describe '.no_fiscal_value?' do
    ...
    context 'when XML is unauthorized' do
      it 'returns true' do
        expect(BrDanfe::DanfeNfeLib::Helper.no_fiscal_value?(xml_unauthorized)).to eq true
      end
    end

    context 'when XML is in homologation environment' do
      it 'returns true' do
        expect(BrDanfe::DanfeNfeLib::Helper.no_fiscal_value?(xml_homologation)).to eq true
      end
    end

    context 'when XML is authorized' do
      it 'returns false' do
        expect(BrDanfe::DanfeNfeLib::Helper.no_fiscal_value?(xml_authorized)).to eq false
      end
    end
  end
  ...
  describe '.address_is_too_big' do
    ...
    context 'when the address is too big for the street field at DANFE' do
      ...
      it 'returns true' do
        expect(BrDanfe::DanfeNfeLib::Helper.address_is_too_big(pdf, address)).to be true
      end
    end
    ...
    context 'when the address fits in the street field in DANFE' do
      ...
      it 'returns false' do
        expect(BrDanfe::DanfeNfeLib::Helper.address_is_too_big(pdf, address)).to be false
      end
    end
  end
  ...
  describe '.generate_address' do
    ...
    it 'returns the address with the street, number and complement' do
      expect(BrDanfe::DanfeNfeLib::Helper.generate_address(xml_street))
