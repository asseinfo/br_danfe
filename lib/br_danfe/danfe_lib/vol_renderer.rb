module BrDanfe
  module DanfeLib
    # Helper to render volumes in additional data on DANFE
    class VolRenderer
      def self.render_fields(det, y_position)
        render_item_titles(y_position)
        render_item_fields(det, y_position)
      end

      def render_item_titles(y_position)
      ...
      def render_item_fields(det, y_position)
      ...
      def render_peso_item_field(val, width, x_pos, y_position)
        @pdf.inumeric 0.35, width, x_pos, y_position, '', val, style_decimal
      end
